...
const getSortedIntersections = (array1, array2) => {
  ...
  array2.forEach(point => {
    if (leftSet.has(`${point.x}|${point.y}`)) {
      let array1Match = array1.find(pointLeft => {
        return pointLeft.x === point.x && pointLeft.y === point.y
      })
      intersections.push({
        x: point.x,
        y: point.y,
        stepsLeft: array1Match.totalSteps,
        stepsRight: point.totalSteps
      })
    }
  })
  return intersections.sort((a, b) => {
    return (Math.abs(a.stepsLeft) + Math.abs(a.stepsRight)) - (Math.abs(b.stepsLeft) + Math.abs(b.stepsRight))
  })
}
...
console.log(`Answer is: ${intersections[0].stepsLeft + intersections[0].stepsRight}`)
