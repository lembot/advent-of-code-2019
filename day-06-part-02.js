...
class OrbitStructure {
  ...
  getAllOrbitsByName(identifier) {
    ...
    while (pos !== -1) {
      path.push(this.relations[pos].parent)
      pos = this.relations.findIndex(obj => {
        return obj.child === this.relations[pos].parent
      })
    } 
  ...
  getDistToSan() {
    ...
    let youMinusSan = youPath.filter(identifier => {
      return !sanPath.includes(identifier)
    })
    let sanMinusYou = sanPath.filter(identifier => {
      return !youPath.includes(identifier)
    })

    return youMinusSan.length + sanMinusYou.length
  }
}
